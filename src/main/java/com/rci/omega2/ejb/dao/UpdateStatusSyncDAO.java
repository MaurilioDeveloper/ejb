...
public class UpdateStatusSyncDAO extends BaseDAO{
    
    private static final Logger LOGGER = LogManager.getLogger(UpdateStatusSyncDAO.class);
    ...
    public void updateStatusSyncState(String adp, StatusSyncStateEnum statusSyncState) throws UnexpectedException{
        ...
        try {
            ...
            sql.append("    UPDATE DossierEntity ds                                                            ");
            sql.append("        SET ds.statusSyncState = :statusSyncState, ds.changeDate = :changeDate         ");
            sql.append("        WHERE ds.adp = :adp                                                            ");
            
            Query query = super.getEntityManager().createQuery(sql.toString());
            query.setParameter("adp", adp);
            query.setParameter("statusSyncState", statusSyncState);
            query.setParameter("changeDate", Calendar.getInstance(new Locale("pr", "BR")).getTime());
            query.executeUpdate();   
